    ...
    : height_(h), width_(w), entrance_(entrance), exit_({-1, -1})
{
    ...
    events_ = new GameEvent**[height_];
    for (uint32_t y = 0; y < height_; y++)
    {
        events_[y] = new GameEvent*[width_]{nullptr};  
    }
}
...
void field::set_event_at(int x, int y, GameEvent* event)
{
...
GameEvent* field::get_event_at(int x, int y) const
{
    if (x >= 0 && x < width_ && y >= 0 && y < height_)
    {
        return events_[y][x];
